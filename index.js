                ...
                if(self.domainType[key] !== 'Categorical') d3.select(this).call(axis.scale(y_axes[key]));
                    ...
                    self.domain[key].forEach((category, index) => {
                        let g = d3.select(this).append('g').attr('class', 'categoricalAxis').attr('category', category)
                            .attr('transform','translate(0,'+index* self.axesSizesWithNoPadding[key] +')');

                        let s = d3.scale.linear().domain([0,1]).range([0, self.axesSizesWithPadding[key]]);
                        g.call(d3.svg.axis().orient('left').scale(s).ticks(0));
                        g.append('text').text(category).attr('x', -10).attr('y', self.y[key](category) + self.axesSizesWithNoPadding[key] / 2);
            ...
            .each(function(key) {
                if(self.domainType[key] !== 'Categorical') d3.select(this).call(axis.scale(y_axes[key]));
                    ...
                    let g = d3.select(this).selectAll('.categoricalAxis')
                        .attr('transform', (d, i) => 'translate(0,'+i* self.axesSizesWithNoPadding[key] +')');
                    ...
                    g.append('text').text(function() {return d3.select(this.parentNode).attr('category')})
                        ...
                        .attr('x', -5)
                        .attr('y', self.axesSizesWithNoPadding[key] / 2);
                }
            });
