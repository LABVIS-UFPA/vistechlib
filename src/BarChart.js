...
class BarChart extends Visualization {
  ...
  resize() {
    ...
    this.settings.filter
      ? (this.keys_filter = this.settings.filter)
      : (this.keys_filter = this.keys);
    this.boxHeight =
      (svgBounds.height - pt - pb - ip * (this.keys_filter.length - 1)) /
      this.keys_filter.length;
    ...
    for (let k of this.keys_filter) {
      ...
      this.y[k].range([this.boxHeight - (type === "Categorical" ? 10 : 0), 0]);
    }
  ...
  data(d) {
    ...
    let xdomain_array = [];
    for (let i = 0; i < this.d.length; i++) xdomain_array.push(i);
    this.x.domain(xdomain_array).range([0, this.innerWidth]);
    this.y = {};
    ...
    for (let k of this.keys_filter) {
      ...
      this.y[k]
        ...
        .range([this.boxHeight - (type === "Categorical" ? 10 : 0), 0]);
    }
  ...
  redraw() {
    ...
    let group_join = this.foreground
      .selectAll("g.dataGroup")
      .data(this.keys_filter, (d) => d)
      .join((enter) => {
        let enter_result = enter.append("g").attr("class", "dataGroup");
        enter_result
          .append("text")
      ...
      .attr(
        "transform",
        (d, i) => `translate(0,${i * this.boxHeight + i * ip})`
      );
    ...
    group_join
      ...
      .attr("y2", "0");
    group_join
      .selectAll(".rule.bottom")
      .attr("x1", "0")
      .attr("y1", barchart.boxHeight)
      .attr("x2", barchart.innerWidth)
      .attr("y2", barchart.boxHeight);
  ...
  detail(...args) {
    ...
    if (args[0] instanceof SVGElement) {
      ...
      args[1] >= 0 &&
      args[1] < this.d.length
    ) {
      details = this.foreground
        .selectAll(`.data[data-index="${args[1]}"]`)
  ...
  removeHighlight(...args) {
    if (args[1] instanceof SVGElement) {
      ...
      let dataSelect = this.foreground
        .selectAll(`.data[data-index="${args[1]}"]`)
        .style("stroke", "none");
  ...
  getHighlightElement(i) {
    ...
    this.foreground.selectAll('rect[data-index="' + i + '"]').each(function () {
      ...
      let rect = d3
        .select(document.createElementNS("http://www.w3.org/2000/svg", "rect"))
  ...
  default: {
    draw: (barchart) => {
      barchart.foreground.selectAll("g.dataGroup").each(function (key) {
        ...
        barchart.settings.gap =
          barchart.x(1) - barchart.x.bandwidth() - barchart.x(0);
    ...
    data: (barchart) => {
      ...
      for (let k of barchart.keys_filter) {
        ...
        barchart.y[k] = d3
          ...
          .range([
            barchart.boxHeight,
            barchart.boxHeight * barchart.breakPoint + barchart.gapSize / 2,
          ]);
        barchart.ybreak[k] = d3
          .scaleLinear()
          .domain([cortefinal, maximo])
          .range([
            barchart.boxHeight * barchart.breakPoint - barchart.gapSize / 2,
            10,
          ]);

        barchart.boxHeightBreak =
          barchart.boxHeight * barchart.breakPoint - barchart.gapSize / 2;
      }
    },
    draw: (barchart) => {
      barchart.foreground.selectAll("g.dataGroup").each(function (key) {
        ...
        g.selectAll("rect.lower")
          ...
          .attr("width", barchart.x.bandwidth())
          .attr("height", (d) =>
            Math.min(barchart.boxHeight - barchart.y[key](d[key]), maxh)
          )
          .style("fill", barchart.settings.color);
        ...
        g.selectAll("rect.upper")
          ...
          .attr("height", (d) =>
            Math.max(barchart.boxHeightBreak - barchart.ybreak[key](d[key]), 0)
          )
          .style("fill", barchart.settings.color);
        barchart.settings.gap =
          barchart.x(1) - barchart.x.bandwidth() - barchart.x(0);
      ...
      barchart.foreground.selectAll("g.dataGroup").each(function (key) {
        ...
        g.selectAll("path")
          ...
          .attr("d", (d, i) => {
            ...
            return `M${x + width / 3},${y} L${x + (width - width / 3)},${y} L${
              x + width
  ...
  "perspective escalonada": {
    data: (barchart) => {
      ...
      for (let k of barchart.keys_filter) {
        ...
        barchart.boxHeightBreak2 =
          barchart.boxHeightBreak * barchart.breakPoint2;

        barchart.y[k] = d3
          .scaleLinear()
          .domain([0, corte])
          .range([barchart.boxHeight, barchart.boxHeightBreak]);
        barchart.ybreak[k] = d3
          .scaleLinear()
          .domain([corte, meio])
          .range([barchart.boxHeightBreak, barchart.boxHeightBreak2]);
        barchart.ybreak2[k] = d3
          .scaleLinear()
          .domain([meio, maximo])
          .range([barchart.boxHeightBreak2, 10]);
      }
    },
    draw: (barchart) => {
      barchart.foreground.selectAll("g.dataGroup").each(function (key) {
        ...
        g.selectAll("rect.lower")
          ...
          .attr("x", (d, i) => barchart.x(i))
          .attr("y", (d) =>
            Math.max(barchart.y[key](d[key]), barchart.boxHeightBreak)
          )
          .attr("width", barchart.x.bandwidth())
          .attr("height", (d) =>
            Math.min(barchart.boxHeight - barchart.y[key](d[key]), maxh)
        ...
        g.selectAll("path.meio")
          ...
          .attr("d", (d, i) => {
            ...
            let width = barchart.x.bandwidth();
            let y = Math.max(
              barchart.ybreak[key](d[key]),
              barchart.boxHeightBreak2
            );
            let height = Math.max(
              Math.min(
                barchart.boxHeightBreak - barchart.ybreak[key](d[key]),
                maxh2
              ),
              0
            );
            return `M${x + width / 4},${y} L${x + (width - width / 3)},${y} L${
              x + width
            },${y + height} L${x},${y + height} Z`;
          });
        ...
        g.selectAll("path.upper")
          ...
          .attr("d", (d, i) => {
            ...
            let width = barchart.x.bandwidth();
            let height = Math.max(
              barchart.boxHeightBreak2 - barchart.ybreak2[key](d[key]),
              0
            );
            return `M${x + width / 4},${y} L${x + (width - width / 3)},${y} L${
              x + (width - width / 3)
            },${y + height} L${x + width / 4},${y + height} Z`;
          });
  ...
  "scale break perspective": {
    data: (barchart) => {
      ...
      for (let k of barchart.keys_filter) {
        ...
        barchart.boxHeightBreak2 =
          barchart.boxHeightBreak * barchart.breakPoint2;
        barchart.boxHeightBreak3 =
          barchart.boxHeightBreak2 * barchart.breakPoint3;
        barchart.boxHeightBreak4 =
          barchart.boxHeightBreak3 * barchart.breakPoint4;
        ...
        barchart.ybreak2[k] = d3
          .scaleLinear()
          .domain([corte2, corte3])
          .range([barchart.boxHeightBreak2, barchart.boxHeightBreak3]);
        barchart.ybreak3[k] = d3
          .scaleLinear()
          .domain([corte3, cortefinal])
          .range([barchart.boxHeightBreak3, barchart.boxHeightBreak4]);
        barchart.ybreak4[k] = d3
          .scaleLinear()
          .domain([cortefinal, maximo])
          .range([barchart.boxHeightBreak4, 10]);
      }
    },
    draw: (barchart) => {
      barchart.foreground.selectAll("g.dataGroup").each(function (key) {
        ...
        g.selectAll("path.meio1")
          ...
          .attr("d", (d, i) => {
            ...
            return `M${x + width * barchart.z},${y} L${
              x + width * (1 - barchart.z)
            },${y} L${x + width},${y + height} L${x},${y + height} Z`;
          });
        ...
        g.selectAll("path.meio2")
          ...
          .attr("d", (d, i) => {
            ...
            return `M${x + width * barchart.z},${y} L${
              x + width * (1 - barchart.z)
            },${y} L${x + width * (1 - barchart.z)},${y + height} L${
              x + width * barchart.z
            },${y + height} Z`;
          });
        ...
        g.selectAll("path.meio3")
          ...
          .attr("d", (d, i) => {
            ...
            let width = barchart.x.bandwidth();
            let y = Math.max(
              barchart.ybreak3[key](d[key]),
              barchart.boxHeightBreak4
            );
            let height = Math.max(
              Math.min(
                barchart.boxHeightBreak3 - barchart.ybreak3[key](d[key]),
                maxh4
              ),
              0
            );
            return `M${x},${y} L${x + width},${y} L${
              x + width * (1 - barchart.z)
            },${y + height} L${x + width * barchart.z},${y + height} Z`;
          });
        ...
        g.selectAll("path.upper")
          ...
          .attr("d", (d, i) => {
            ...
            let width = barchart.x.bandwidth();
            let height = Math.max(
              barchart.boxHeightBreak4 - barchart.ybreak4[key](d[key]),
              0
            );
        ...
        g.selectAll(
          "g.y.loweraxis, g.y.meio1, g.y.meio2, g.y.meio3, g.y.upper, g.Axisright.meio1, g.Axisright.meio2, g.Axisright.meio3, g.Axisright.meio4, g.Axisright.meio5, g.Line1, g.Line2, g.Line3, g.Line4, g.Line5"
        ).remove();
        ...
        for (let j = 0; j < 4; j++) {
          g.append("path")
            ...
            .attr("d", (d, i) => {
              ...
              y =
                barchart.boxHeightBreak +
                j * ((barchart.boxHeightBreak2 - barchart.boxHeightBreak) / 3);
        ...
        for (j = 1; j < 6; j++) {
          g.append("path")
            ...
            .attr("d", (d, i) => {
              ...
              let x2 =
                barchart.innerWidth - barchart.x.bandwidth() * barchart.z;
              let y =
                barchart.boxHeightBreak2 +
                j * ((barchart.boxHeightBreak3 - barchart.boxHeightBreak2) / 5);
        ...
        for (j = 0; j < 4; j++) {
          g.append("path")
            .attr("stroke", "black")
            .attr("stroke-width", 0.1 + (j * 27) / 100)
            .attr("class", "Line4")
            .attr("d", (d, i) => {
              let x2 = barchart.innerWidth - xbreak3;
              let y =
                barchart.boxHeightBreak3 +
                j * ((barchart.boxHeightBreak4 - barchart.boxHeightBreak3) / 3);
