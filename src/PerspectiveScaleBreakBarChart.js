...
class PerspectiveScaleBreakBarChart extends Visualization {
    constructor(parentElement, settings) {
        ...
        this.webglContainer = d3.select(this.parentElement)
            .append("div")
    ...
    resize() {
        ...
        this.settings.width = this.settings.size_type === "fit" ? bounds.width : this.settings.width;
        this.settings.height = this.settings.size_type === "fit" ? bounds.height : this.settings.height;
    ...
    updateDepth(depth) {
        this.settings.depth = Math.max(this.settings.minDepth, Math.min(this.settings.maxDepth, depth));
        this._renderBars(this.settings.depth);
        return this;
    }
    ...
    _updateCamera(createNew) {
        ...
        let viewSize = Math.max(this.settings.cameraViewSize, verticalView, horizontalView);
        ...
        if (createNew || !this.camera) {
            ...
            this.camera.position.set(this.settings.cameraX, this.settings.cameraY, this.settings.cameraZ);
    ...
    _getVisualHeight(d, depth) {
        ...
        let t = (value - this.minNonOutlier) / (this.maxNonOutlier - this.minNonOutlier || 1);
    ...
    _renderBars(depth) {
        ...
        this.d.forEach((d, i) => {
            ...
            let value = +d[this.valueKey];
            let bar = this.hasOutlier && value === this.maxValue
    ...
    _createNormalBar(x, height) {
        let mesh = new THREE.Mesh(
            new THREE.BoxGeometry(this.settings.barWidth, height, this.settings.barDepth),
            this.material
        );
        mesh.position.set(x, height / 2 + 0.05, 0);
        return mesh;
    }
    ...
    _createFoldedBar(x, totalHeight, depth) {
        ...
        if (depth <= 0 || totalHeight <= this.settings.breakEnd) {
            group.add(this._createNormalBar(0, totalHeight));
            return group;
        }

        let upperHeight = Math.max(0.1, totalHeight - this.settings.breakStart - this.settings.foldVisualHeight);
        let y0 = 0;
        ...
        let geometry = this._createPrismFromYZProfile(points, this.settings.barWidth, this.settings.barDepth);
